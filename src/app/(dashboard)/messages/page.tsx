...
export default function MessagesPage() {
      ...
      <div className="flex-1 overflow-y-auto">
        {conversations.map((c) => (
          <div key={c.partner.id} onClick={() => openConv(c)}
            ...
            {c.unread > 0 && (
              <div className="w-5 h-5 rounded-full bg-accent text-[9px] font-bold text-white flex items-center justify-center flex-shrink-0 self-center">{c.unread}</div>
            )}
          </div>
        ))}
  ...
  const ChatArea = () => (
    <div className="flex flex-col h-full bg-bg">
      ...
      <div className="px-4 py-3.5 border-b border-border flex items-center gap-3 bg-surface flex-shrink-0">
        ...
        <button onClick={() => setMobileView("list")} className="md:hidden text-text-muted hover:text-text-main mr-1 text-lg">←</button>
        ...
        {messages.map((m) => {
          ...
          return (
            <div key={m.id} className={`flex ${isMe ? "justify-end" : "justify-start"}`}>
              <div className={`max-w-[75%] md:max-w-[65%] px-4 py-2.5 rounded-2xl text-[13px]
      ...
      <div className="p-3 border-t border-border flex gap-2 flex-shrink-0 bg-surface">
        ...
        <button onClick={sendMessage} disabled={sending || !newMsg.trim()} className="btn-primary px-4 text-sm">
          {sending ? "…" : "→"}
        </button>
      </div>
    </div>
  ...
  return (
    <>
      ...
      <div className="hidden md:flex max-w-6xl h-[calc(100vh-120px)] min-h-[500px] rounded-2xl overflow-hidden border border-border">
        <div className="w-72 flex-shrink-0 bg-surface border-r border-border">
          <ConversationList />
        </div>
        {activeConv ? <ChatArea /> : <EmptyState />}
      </div>

      {/* ── MOBILE layout: full screen, switch between list and chat ── */}
      <div className="md:hidden flex flex-col" style={{ height: "calc(100vh - 112px)" }}>
        {mobileView === "list" ? (
          <div className="flex-1 bg-surface rounded-2xl border border-border overflow-hidden">
            <ConversationList />
          </div>
        ) : activeConv ? (
