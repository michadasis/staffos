...
const NAV_ALL = [
  ...
  { href: "/attendance", icon: "📅", label: "Attendance", roles: ["ADMIN", "MANAGER", "STAFF"] },
  { href: "/leave", icon: "🌴", label: "Leave", roles: ["ADMIN", "MANAGER", "STAFF"] },
  { href: "/payroll", icon: "💳", label: "Payroll", roles: ["ADMIN", "MANAGER", "STAFF"], payrollOnly: true },
...
// Desktop sidebar
function Sidebar({ collapsed, onToggle, payrollEnabled }: { collapsed: boolean; onToggle: () => void; payrollEnabled: boolean }) {
  const pathname = usePathname();
  const { user, logout } = useAuth();
  const role = user?.role || "STAFF";
  const nav = NAV_ALL.filter((n) => n.roles.includes(role) && (!(n as any).payrollOnly || payrollEnabled));
...
function MobileDrawer({ open, onClose, payrollEnabled }: { open: boolean; onClose: () => void; payrollEnabled: boolean }) {
  const pathname = usePathname();
  const { user, logout } = useAuth();
  const role = user?.role || "STAFF";
  const nav = NAV_ALL.filter((n) => n.roles.includes(role) && (!(n as any).payrollOnly || payrollEnabled));
...
function DashboardInner({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    fetch("/api/system-settings").then(r => r.json()).then(({ data }) => {
      if (data?.payrollEnabled === "true") setPayrollEnabled(true);
    }).catch(() => {});
  }, []);
  ...
  return (
    <div className="flex h-screen overflow-hidden">
      {/* Desktop sidebar */}
      <Sidebar collapsed={collapsed} onToggle={() => setCollapsed((c) => !c)} payrollEnabled={payrollEnabled} />
      ...
      {/* Mobile bottom nav */}
      <BottomNav payrollEnabled={payrollEnabled} />
    </div>
  );
}
