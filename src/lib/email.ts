  ...
  <table width="100%" cellpadding="0" cellspacing="0" style="background:#0f172a;padding:40px 20px">
    <tr><td align="center">
      <table width="580" cellpadding="0" cellspacing="0" style="max-width:580px;width:100%">
        <tr>
          <td style="background:#1e293b;border-radius:16px 16px 0 0;padding:24px 32px;border-bottom:1px solid #334155">
            <span style="font-size:20px;font-weight:800;color:#f1f5f9;letter-spacing:-0.5px">StaffOS</span>
          </td>
        </tr>
        <tr>
        ...
        <tr>
          <td style="background:#0f172a;border-radius:0 0 16px 16px;padding:20px 32px;border-top:1px solid #1e293b">
            <p style="margin:0;font-size:11px;color:#475569;text-align:center">
              You're receiving this because you have an account on <a href="${APP_URL}" style="color:#3b82f6;text-decoration:none">StaffOS</a>.
            </p>
          </td>
        </tr>
      </table>
...
function btn(text: string, url: string, color = "#3b82f6") {
  return `<a href="${url}" style="display:inline-block;background:${color};color:#fff;font-size:13px;font-weight:700;padding:12px 24px;border-radius:10px;text-decoration:none;margin-top:24px">${text}</a>`;
}
function h1(text: string) {
  return `<h1 style="margin:0 0 8px;font-size:22px;font-weight:800;color:#f1f5f9">${text}</h1>`;
}
function p(text: string) {
  return `<p style="margin:0 0 12px;font-size:14px;color:#94a3b8;line-height:1.6">${text}</p>`;
}
function badge(text: string, color = "#3b82f6") {
  return `<span style="display:inline-block;background:${color}20;color:${color};border:1px solid ${color}30;font-size:11px;font-weight:700;padding:3px 10px;border-radius:999px">${text}</span>`;
}
function divider() {
...
export async function sendTaskAssignedEmail(to: string, recipientName: string, taskTitle: string, taskId: number, deadline?: Date | null, priority?: string) {
  ...
  await send(to, `New task assigned: ${taskTitle}`, layout(`
    ...
    ${p(`Hello ${recipientName}, a new task has been assigned to you. Please review it and update the status as you progress.`)}
...
export async function sendOverdueTaskEmail(to: string, recipientName: string, tasks: { title: string; deadline: Date }[]) {
  ...
  await send(to, `You have ${tasks.length} overdue task${tasks.length > 1 ? "s" : ""}`, layout(`
    ...
    ${p(`Hello ${recipientName}, the following task${tasks.length > 1 ? "s are" : " is"} past the deadline:`)}
...
export async function sendPendingRegistrationEmail(to: string, adminName: string, applicantName: string, applicantEmail: string) {
  await send(to, `New registration pending approval: ${applicantName}`, layout(`
...
export async function sendRegistrationApprovedEmail(to: string, name: string) {
  await send(to, "Your registration has been approved", layout(`
    ...
    ${p(`Hello ${name}, your registration has been approved. You can now sign in and access the platform.`)}
  ...
  await send(to, `Email change request from ${requesterName}`, layout(`
    ...
    <div style="background:#0f172a;border:1px solid #334155;border-radius:12px;padding:20px;margin:16px 0">
      <table width="100%" cellpadding="0" cellspacing="0">
        <tr>
          <td style="font-size:12px;color:#64748b;padding-bottom:6px">Current</td>
          <td style="font-size:13px;color:#f1f5f9;text-align:right;text-decoration:line-through">${currentEmail}</td>
        </tr>
        <tr>
          <td style="font-size:12px;color:#64748b">New</td>
          <td style="font-size:13px;color:#3b82f6;text-align:right;font-weight:700">${newEmail}</td>
        </tr>
      </table>
    </div>
...
export async function sendEmailChangeApprovedEmail(to: string, name: string, newEmail: string) {
  await send(to, "Your email change has been approved", layout(`
    ...
    ${p(`Hello ${name}, your email address has been updated to <strong style="color:#f1f5f9">${newEmail}</strong>.`)}
...
export async function sendPasswordChangedEmail(to: string, name: string) {
  await send(to, "Your password was changed", layout(`
    ...
    ${p("If you made this change, no action is needed. If you did not, contact your administrator immediately.")}
    ${divider()}
...
export async function sendTwoFactorChangedEmail(to: string, name: string, enabled: boolean) {
  ...
  await send(to, `Two-factor authentication ${action}`, layout(`
    ...
    ${p("If you did not make this change, please contact your administrator immediately and change your password.")}
    ${divider()}
    ${p(`<span style="font-size:12px;color:#64748b">Changed at: ${new Date().toLocaleString()}</span>`)}
  `));
}
...
export async function sendOverdueTaskEmail(to: string, recipientName: string, tasks: { title: string; deadline: Date }[]) {
  ...
  await send(to, `You have ${tasks.length} overdue task${tasks.length > 1 ? "s" : ""}`, layout(`
    ...
    <table width="100%" cellpadding="0" cellspacing="0" style="background:#0f172a;border:1px solid #334155;border-radius:12px;overflow:hidden;margin:16px 0">
      <thead>
        <tr style="background:#1a2540">
          <th style="padding:10px 12px;font-size:11px;color:#64748b;text-align:left;font-weight:700;text-transform:uppercase;letter-spacing:0.05em">Task</th>
          <th style="padding:10px 12px;font-size:11px;color:#64748b;text-align:left;font-weight:700;text-transform:uppercase;letter-spacing:0.05em">Deadline</th>
        </tr>
      </thead>
      <tbody>${taskList}</tbody>
    </table>
...
export async function sendEmailChangeRequestEmail(to: string, adminName: string, requesterName: string, currentEmail: string, newEmail: string) {
  await send(to, `Email change request from ${requesterName}`, layout(`
    ...
    <div style="background:#0f172a;border:1px solid #334155;border-radius:12px;padding:20px;margin:16px 0">
      <table width="100%" cellpadding="0" cellspacing="0">
        <tr>
          <td style="font-size:12px;color:#64748b;padding-bottom:6px">Current email</td>
          <td style="font-size:13px;color:#f1f5f9;text-align:right;text-decoration:line-through">${currentEmail}</td>
        </tr>
        <tr>
          <td style="font-size:12px;color:#64748b">New email</td>
          <td style="font-size:13px;color:#3b82f6;text-align:right;font-weight:700">${newEmail}</td>
        </tr>
      </table>
    </div>
    ${btn("Review Request", `${APP_URL}/staff`, "#3b82f6")}
  `));
}
...
export async function sendEmailChangeApprovedEmail(to: string, name: string, newEmail: string) {
  await send(to, "Your email change has been approved", layout(`
    ${h1("Email change approved")}
    ${p(`Hello ${name}, your email address has been updated successfully.`)}
    <div style="background:#0f172a;border:1px solid #334155;border-radius:12px;padding:16px 20px;margin:16px 0">
      <p style="margin:0;font-size:13px;color:#94a3b8">New email address: <span style="color:#f1f5f9;font-weight:700">${newEmail}</span></p>
    </div>
    ${p("Please use your new email address to sign in from now on.")}
    ${btn("Sign In", `${APP_URL}/login`)}
  `));
...
export async function sendPasswordChangedEmail(to: string, name: string) {
  await send(to, "Your password was changed", layout(`
    ${h1("Password changed")}
    ${p(`Hello ${name}, your StaffOS password was just changed.`)}
    ${p("If you made this change, no action is needed. If you did not change your password, please contact your administrator immediately.")}
    ${divider()}
    ${p(`<span style="font-size:12px;color:#64748b">Changed at: ${new Date().toLocaleString()}</span>`)}
  `));
}
